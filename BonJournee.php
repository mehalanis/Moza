...
if(isset($_GET["idVendeur"])){
  ...
  $result=$database->query("select produit.id_produit as idproduit,produit.nom as nomproduit,stock_vehicule.quantite
                        as qte_init ,dd.id_produit_prix as dd_id_prix,dd.prix as dd_prix,dg.id_produit_prix as dg_id_prix
                         ...
                         group by produit_prix.id_produit)
                         left join stock_vehicule on stock_vehicule.id_vehicule=".$Vendeur["id_vehicule"]." and produit.id_produit=stock_vehicule.id_produit order by idproduit ASC");
}
...
where journee.id_journee=".$_GET["id_journee"]."  group by journee.id_journee;");
  $Vendeur=mysqli_fetch_assoc($result);
  $result=$database->query(" select max(id_journee) as id_journee from journee where id_vendeur=".$Vendeur["idvendeur"]);
  $row=mysqli_fetch_assoc($result);
  $modifier="";
  if($row["id_journee"]!=$_GET["id_journee"]){ $modifier="disabled";}
        ...
        <div class="table">
          <form class="" action="BonJournee.php" method="post">
                ...
                <?php
                 ...
                 ?>
                  <div class="control_table_item_9col">
                    ...
                    <input id="qte_init_<?php echo $row["idproduit"]; ?>"   type="text" disabled class="controlinput" value="<?php  echo $qte_init; ?>">
                    <input id="qte_sortie_<?php echo $row["idproduit"]; ?>"  name="qte_sortie[]" type="text" min="0" onkeyup="calculCharge(<?php echo $row["idproduit"]; ?>,this.value)"  class="controlinput" value="<?php echo $qte_sortie; ?>" autocomplete="off">
                    ...
                    <input id="qte_vendue_dd_<?php echo $row["idproduit"]; ?>" type="text" name="qte_vendue_dd[]" autocomplete="off"
                     ...
                     <input id="qte_vendue_dg_<?php echo $row["idproduit"]; ?>"  type="text" name="qte_vendue_dg[]"  autocomplete="off"
                    ...
                    <input id="qte_vendue_sg_<?php echo $row["idproduit"]; ?>" type="txt" name="qte_vendue_sg[]"  autocomplete="off"
             ...
             <fieldset class="fields">
               ...
               <div class="control_table">
                 <div class="control_table_item_6col" >
                   ...
                   <input id="facture" type="number" disabled class="controlinput" value="<?php if(isset($_GET["id_journee"])) echo $Vendeur["facture"]; ?>">
                   <label class="controllabel" for="" >Recette</label>
                   <input id="recette" type="number" onkeyup="_ecart()" name="recette"  class="controlinput" value="<?php if(isset($_GET["id_journee"])) echo $Vendeur["recette"]; ?>">
                   <label class="controllabel" for="" >Ecart</label>
                   <input id="ecart" type="number" disabled class="controlinput" value="<?php if(isset($_GET["id_journee"])) echo $Vendeur["ecart"]; ?>">
                 </div>
               </div>
             </fieldset>
           </div>
           ...
           <div class="control_div_btn">
             ...
             <button type="submit" class="control_btn" <?php if(isset($modifier)) echo $modifier; ?> name="<?php if(isset($_GET["idVendeur"])) echo "Ajouter";
